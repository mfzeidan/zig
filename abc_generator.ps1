...
ORDER BY 2
    ,3;"
$sqlFileDirectory = "C:\Users\student\Documents\polarexpress\version 1\ABC"
...
$section1FilePath = Join-Path $sqlFileDirectory "METADATA.ETLAuditBalanceControl.sql"
...
# Get X and Y values
$section1Rows = $mainTable.Select("section_id = 1")
$X = $section1Rows[0]["Insert Statement"].Split('(')[1].Split(',')[0]
$Y = $section1Rows[1]["Insert Statement"].Split('(')[1].Split(',')[0]



# Generate the header for the section 1 SQL file
$headerSection1 = "USE ROLE OWNER_(environment);
...
BEGIN TRANSACTION;
DELETE FROM AUDITBALANCECONTROL 
WHERE AUDITBALANCECONTROL_ID IN ($($X),$($Y));
INSERT INTO AUDITBALANCECONTRL(ABCID,
                                APPNAME,
                                JOBNAME,
                                SEQUENCENUMBER,
                                STATUSCODE,
...
$lastRowInSection1 = $mainTable.Select("section_id = 1")[-1]
...
Write-Host "ABC data exported to $section1FilePath"
